...
export async function inspect(rawInput: InspectInput): Promise<Report> {
  ...
  const cloneImplCode = cloneOf ? await reader.getCode(cloneOf).catch(() => null) : null;
  ...
  if (cloneOf === null) {
    ...
    // The clone's own claim (its trampoline matches EIP-1167) is trusted for the proxy verdict,
    // but its implementation code couldn't be fetched, so nothing downstream can be read either.
    logicCode = null;
  } else {
  ...
  const hasPrivileged = (found?.length ?? 0) > 0;

  const findings = await Promise.all([
    guard("verified", () => checkVerified(input, contract ? contract.verified : null)),
    guard("ownership", () => checkOwnership(input, owner, hasPrivileged)),
    ...
    guard("proxy", () => {
      ...
      return checkProxy(input, { cloneOf, cloneTargetIsProxy, forwardsToUnidentifiedCode, topSlotsSet });
    }),
